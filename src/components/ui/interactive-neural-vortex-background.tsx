...
export const InteractiveNeuralVortex = () => {
    ...
    `;
        ...
        window.addEventListener('pointermove', handleMouseMove as unknown as EventListener);
        window.addEventListener('touchmove', handleTouchMove as unknown as EventListener);

        // Cleanup
        return () => {
            window.removeEventListener('resize', resizeCanvas);
            window.removeEventListener('pointermove', handleMouseMove as unknown as EventListener);
    ...
    return (
        <canvas
            ref={canvasRef}
            id="neuro"
            className="fixed inset-0 w-full h-full pointer-events-none opacity-[0.4] -z-50"
        ></canvas>
    );
};
