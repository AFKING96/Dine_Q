...
export function AnimeNavBar({ items }: NavBarProps) {
  ...
  // Effect for active tab sync
  useEffect(() => {
    const current = items.find(item => pathname === item.url)
    if (current) {
      // eslint-disable-next-line
              ...
              >
                {isActive && (
                  <div className="absolute inset-0 rounded-full -z-10">
                    <div className="absolute inset-0 bg-gradient-to-r from-violet-600 to-purple-600 rounded-full shadow-lg" />
                  </div>
                )}
